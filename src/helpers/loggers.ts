...
export const blue = (str: string | number | undefined) =>
  `${ansiCodes.blue}${str}${ansiCodes.end}`
export const yellow = (str: string | number | undefined) =>
  `${ansiCodes.yellow}${str}${ansiCodes.end}`

const getTitle = (
  workers: NodeJS.Dict<import('cluster').Worker> | undefined
) => {
...
export const logPrimaryServerProxyRequest = (
  method = '',
  port: number,
  url = ''
) => {
  console.log(
    `${blue('[PRIMARY SERVER]')} Proxy request [${method}] ${port}${url}`
  )
}
